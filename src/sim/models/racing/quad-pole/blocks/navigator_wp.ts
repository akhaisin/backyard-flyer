...
//   2. Acceleration  → desired attitude (yaw-aware, same math as the old
//                      attitude-PID navigator)
...
//   Hover ≈ 0.245 at MASS=1 g=9.81 MAX_THRUST_N=10.
//
// When missionType !== MISSION_WP this block passes through the current aetr
// unchanged and zeros its position integral.
...
// matches the legacy attitude PID's KP_ATT. fc_acro's KP_RATE is held at 0.05
// to keep stick deflection-to-rate response gentle for the pilot path;
// the autonomous path compensates with a high outer gain that often saturates
// the rate stick (clamped at ±1) for non-trivial attitude errors. That's fine —
// it means autonomous flight asks for the max rate fc_acro allows.
...
export function navigator_wp(state: NavIn): NavOut {
  ...
  if (!state.armed || Math.round(state.missionType) !== MISSION_WP) {
