...
function entrySegStart(winIdx: number): Vec3 {
  return winIdx > 0 ? WINDOWS_B[winIdx - 1].center : HOME;
}
...
export function mission_b(state: MissionIn): MissionOut {
  ...
  if (phase === RTH) {
    ...
    return out(RTH, winIdx, ticks + 1, 1, 0, HOME, n0, d, loops,
               WINDOWS_B[WINDOWS_B.length - 1].center, HOME);
  }
  ...
  if (phase === MISSED) {
    ...
    return out(MISSED, winIdx, ticks + 1, 1, 0, recoveryCenter, WINDOWS_B[winIdx].normal, d, loops);
  }
