...
//
// Key difference from quad-w1b: roll/pitch commands are decomposed in the drone's
// body frame rather than always assuming yaw=0. With arbitrary yaw ψ, the small-angle
// mapping from world-frame acceleration demands to body pitch/roll is:
//
...
// The derivation: the rotation matrix A=[[-cp,sp],[sp,cp]] mapping [θ,φ]→[ax,az]/g
// is its own inverse (det=−1), giving the expressions above.
//
// The yawSetpoint from the path planner is passed straight through to the stabilizer.
