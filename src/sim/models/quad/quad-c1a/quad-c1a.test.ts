...
import { quadC1aConfig } from './quad-c1a.config';
import type { ModelState } from '../../../engine/types';

// Block Three.js / WebGL imports that fail in jsdom.
vi.mock('./quad-c1a.vis', () => ({ default: () => null }));

// Seeded LCG — reproducible across runs.
...
function runSimToCompletion(simId: string, config: ReturnType<typeof quadC1aConfig>): void {
...
describe('quad-c1a sim', () => {
  ...
  it('completes 2 intercept rounds within the default budget', () => {
    const simId = 'quad-c1a-default';
    runSimToCompletion(simId, quadC1aConfig());
    assertPass(simId);
  });
});
