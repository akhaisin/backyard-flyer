...
const view = (s: ModelState): QuadC1aState => s as unknown as QuadC1aState;
...
const sceneHandler = composeScene(() => {
  // Shared mutable refs: created once per vis instance in the factory closure.
  const trailRef  = { enabled: true };
  const guidesRef = { enabled: true };
  const TOGGLE_LABEL_W = 44; // px — wide enough for "Guides" so stacked toggles align
  ...
  textLabel({
    text: 'Quad C1a\nDirect-pursuit chase\ntarget_c1 + planner_c1a / navigator_w1',
    position: [-16, 0, 0],
    fontSize: 36,
  }),
...
export default function QuadC1aVis() {
