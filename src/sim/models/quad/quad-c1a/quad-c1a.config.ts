...
import plannerC1aCode from '../../lib/quad/planner_c1a.ts?raw';
import { planner_c1a } from '../../lib/quad/planner_c1a';
...
import { C1A_ROUTE } from './route';
import QuadC1aVis from './quad-c1a.vis';
import type { ModelConfig, ModelState } from '../../../engine/types';

const motors0 = { m0: 0, m1: 0, m2: 0, m3: 0 };
const vec0    = { x: 0, y: 0, z: 0 };

// First step's spawn point — used to initialise target_c1 and planner_c1a
// before the first tick so they carry a valid position from the start.
const STEP0_POS = C1A_ROUTE[0].pos;

export function quadC1aConfig(overrides?: Partial<QuadConsts>): ModelConfig {
  const modelOverrides: Partial<QuadConsts> = {
    // Kinematic target pursuit benefits from the same wind-rejection integral
    // and extra tilt authority as the window-gate models.
    KI_POS:        0.3,
    MAX_TILT:      0.4,
    // Yaw loop: soften + low-pass measurement to suppress heading vibration
    // while the quad tracks a moving carrot.
    KP_YAW_OUTER:  5.0,
    YAW_MEAS_LPF:  0.35,
    // Chase scenario: cross-track error against a fixed segment is not a
    // meaningful metric, so the ceiling is set high enough to never fail.
    ACC_ERR_LIMIT: 999999,
    // Calibrated from a seeded run: ~10 restarts observed across 15 laps. Set to
    // ~50 % headroom over a 2-lap window (~2 restarts expected in 2 laps).
    MAX_RESTARTS: 15,
    // One lap = four successful intercepts (all sides of the square).
    REQUIRED_LAPS: 2,
    MAX_TICKS:     6000,
    simDuration:   8000,
    ...overrides,
  ...
  return {
    ...
    initialState: {
      ...
      mission: {
        ...
        step: {
          pos:       { ...STEP0_POS },
          stepType:  5,
          dest:      { ...C1A_ROUTE[0].dest },
          speed:     C1A_ROUTE[0].speed,
          threshold: C1A_ROUTE[0].threshold,
        },
        target:   { ...vec0 },
        dist:     0,
        segStart: { ...vec0 },
    ...
    blocks: [
      makeLifecycleBlock(QUAD_DEFAULTS, C1A_ROUTE, modelOverrides),
      // --- Environment ---
      {
        sourceId: 'wind',
        exportName: 'wind',
      ...
      // --- Flight stack ---
      {
        ...
        mapStateIn: (s) => ({
          ...
          statusWp:     (s.planner_c1a as ModelState).stepStatus,
          statusReturn: (s.planner_wp as ModelState).stepStatus,
          // Mission sees only step[0] so STATUS_COMPLETED → RTH immediately
          // (next = 1 >= steps.length = 1). target_c1 gets the full steps list
          // and loops through all segments independently.
      ...
      {
        // Kinematic target — moves step.pos → step.dest at step.speed m/s,
        // resets whenever mission is outside PHASE_NAVIGATE.
        sourceId: 'target_c1',
      ...
      {
        // Direct-pursuit planner — carrot = target position.
        sourceId: 'planner_c1a',
        exportName: 'planner_c1a',
        defaultFn: (s) => planner_c1a(s as Parameters<typeof planner_c1a>[0]),
        defaultCode: plannerC1aCode,
        mapStateIn: (s) => ({
          pos:         (s.sensors as ModelState).pos,
          ...
          armed:       (s.mission as ModelState).armed,
          phase:       (s.mission as ModelState).phase,
          yawSetpoint: (s.planner_c1a as ModelState).yawSetpoint,
        }),
        mapStateOut: (out, s) => ({
          ...s,
          planner_c1a: {
      ...
      {
        // Recovery-waypoint checker — reports arrival at the restart anchor
        // (mission's PHASE_RESTART fly-back target) via statusReturn.
        sourceId: 'planner_wp',
      ...
      },
      {
        ...
        mapStateIn: (s) => ({
          ...
          carrot:      (s.planner_c1a as ModelState).carrot,
          yawSetpoint: (s.planner_c1a as ModelState).yawSetpoint,
          armed:       (s.mission as ModelState).armed,
    ...
    blocksDiagram: [
      ...
      { from: 'noise',        to: 'planner_c1a',   label: 'pos'           },
      { from: 'noise',        to: 'navigator_w1',  label: 'sensors'       },
      { from: 'noise',        to: 'fc_acro',       label: 'rates'         },
      { from: 'mission',      to: 'target_c1',     label: 'phase/stepIdx' },
      { from: 'mission',      to: 'planner_c1a',   label: 'step/phase'    },
      { from: 'target_c1',    to: 'planner_c1a',   label: 'target pos'    },
      { from: 'planner_c1a',  to: 'mission',       label: 'status'        },
      { from: 'mission',      to: 'planner_wp',    label: 'anchor'        },
      { from: 'planner_wp',   to: 'mission',       label: 'return'        },
      { from: 'planner_c1a',  to: 'navigator_w1',  label: 'carrot+yaw'   },
