// navigator_c2 — pursuit navigator for the c2 chase-and-intercept family.
//
// Reads the carrot + yawSetpoint from planner_c2a and the sensed world state,
// then emits Mode-2 AETR sticks for fc_acro. Currently identical to navigator_w1
// (carrot-following with outer-P attitude loop + KI_POS wind rejection); this
// block is the extension point for a dedicated interception algorithm.
//
...
//
// All gains/limits arrive via state.K from the lifecycle block.
...
export function navigator_c2(state: NavIn): NavOut {
